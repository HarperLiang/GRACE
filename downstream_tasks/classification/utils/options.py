...
def parse_args():
    ...
    parser.add_argument('--excel_file', type=str, default=None,
                    help='Path to XLSX split file (overrides --csv_file if set)')
    parser.add_argument("--feature", type=str, help="directory (or comma-separated) of .pt features")
    # parser.add_argument("--feature", type=str, help="which feature extractor to use")
    parser.add_argument("--label_column", type=str, default="label")
    parser.add_argument("--split_keep", type=str, default=None,  
                   help="Comma-separated list of cohort/study tags to keep, e.g. 'YNCH-GC,NanFang-GC'")
    # utils/options.py (inside parse_args)
    parser.add_argument(
        "--flip_labels",
        action="store_true",
        help="Swap 0<->1 labels (binary only). Useful when the CSV's label semantics are reversed."
    )
