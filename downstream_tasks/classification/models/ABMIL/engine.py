    ...
    def learning(self, model, loaders, criterion, optimizer, scheduler):
        ...
        if self.args.resume is not None:
            if os.path.isfile(self.args.resume):
                ...
                # ---- 兼容逻辑 ----
                self.val_scores = checkpoint.get("val_scores", checkpoint.get("val_score", None))
                self.best_epoch = checkpoint.get("best_epoch", 0)
                if "test_scores" in checkpoint:
                    self.test_scores = checkpoint["test_scores"]
                elif "test_score" in checkpoint:
                    self.test_scores = checkpoint["test_score"]

                model.load_state_dict(checkpoint["state_dict"])

                # 打印 val score
                if isinstance(self.val_scores, dict) and "Macro_AUC" in self.val_scores:
                    print("=> loaded checkpoint (val score: {})".format(self.val_scores["Macro_AUC"]))
                else:
                    print("=> loaded checkpoint (no val_score info)")

                # 打印 test score
                if isinstance(self.test_scores, dict) and "Macro_AUC" in self.test_scores:
