...
from dataset import GC_FoundationDataset

import warnings

warnings.filterwarnings("ignore")

torchvision_archs = sorted(name for name in torchvision_models.__dict__ if name.islower() and not name.startswith("__") and callable(torchvision_models.__dict__[name]))
...
def get_args_parser():
    ...
    # Misc
    parser.add_argument("--data_path", default="/path/to/data", type=str, help="Please specify path to the ImageNet training data.")
...
def train_dino(args):
    ...
    if args.arch == "uni2":
        ...
        student = timm.create_model("hf-hub:MahmoodLab/UNI2-h", pretrained=False, **timm_kwargs)
        msg = student.load_state_dict(torch.load("/path/to/uni2.bin", map_location="cpu"), strict=True)
        print(msg)
        teacher = copy.deepcopy(student)
        embed_dim = student.embed_dim
    elif args.arch == "uni":
        student = timm.create_model("vit_large_patch16_224", img_size=224, patch_size=16, init_values=1e-5, num_classes=0, dynamic_img_size=True)
        msg = student.load_state_dict(torch.load("/path/to/uni.bin", map_location="cpu"), strict=True)
        print(msg)
        teacher = copy.deepcopy(student)
        embed_dim = student.embed_dim
    elif args.arch == "virchow2":
        student = timm.create_model("hf-hub:paige-ai/Virchow2", pretrained=True, mlp_layer=SwiGLUPacked, act_layer=torch.nn.SiLU)
        teacher = copy.deepcopy(student)
        embed_dim = student.embed_dim
    else:
